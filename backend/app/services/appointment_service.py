...
class AppointmentService:
    ...
    async def get_my_appointments(
        ...
        """Return the calling student's own appointments (upcoming + past)."""
        student = await cls._get_student_by_user_id(db, UUID(current_user["id"]))
        ...
        rows = (
            await db.execute(
                query.order_by(Appointment.start_time.asc()).limit(limit).offset(offset)
            )
        ).all()
